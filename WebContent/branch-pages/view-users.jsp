				...
				try {
					...
					switch (rolex.ordinal()) {
						case 0 :
							...
							break;
						case 1 :
							SQL_SELECT = "SELECT id, title, firstName, lastName, dob, username, role, bankcode, branchcode, reg_status FROM customer WHERE ROLE='BRANCH' OR ROLE='CUSTOMER'";
							break;
						case 2 :
							SQL_SELECT = "SELECT id, title, firstName, lastName, dob, username, role, bankcode, branchcode, reg_status FROM customer WHERE ROLE='CUSTOMER'";
							break;
						case 3 :
							flag = false;
						default :
							...
							for (int i = 1; i <= rs.getMetaData().getColumnCount(); i++) {
								if (rs.getMetaData().getColumnName(i).equalsIgnoreCase("role")) {
									out.println("<td><select id=\"" + id
											+ "_role\" class=\"form-control select-control select\" disabled>");
									...
									out.println("<td>" + (Boolean.parseBoolean(rs.getString(i))
											? "<button id=\"" + id
													+ "_register\" class=\"btn btn-sm btn-default btn-table btn-register\" onclick=\"register('"
													+ rs.getString("id") + "', )\">UPDATE ACCOUNT</button>"
											: "<button id=\"" + id
													+ "_register\" class=\"btn btn-sm btn-success btn-table btn-register\" onclick=\"register('"
													+ rs.getString("id") + "', )\">CREATE ACCOUNT</button>")
											+ "</td>");
									...
									if (!Boolean.parseBoolean(rs.getString("reg_status")) || rs.getString(i) == null) {
										...
										out.println("<td id=\"" + id + "_username\">" + rs.getString(i) + "</td>");
									}
								} else if (rs.getMetaData().getColumnName(i).equalsIgnoreCase("BANKCODE")) {
									out.println("<td><select id=\"" + id
											+ "_bankcode\" class=\"form-control select-control select todisable\" disabled>");
									out.println("<option value=\"null\">None</option>");
									for (Bank bank : banks) {
									...
									out.println("<td><select id=\"" + id
											+ "_branchcode\" class=\"form-control select-control select todisable\" disabled>");
									out.println("</select></td>");
									out.println("<script>$('#" + id
...
<div class="cover"
	style="height: 100%; width: 100%; position: absolute; left: 0; top: 0; background-color: rgba(0, 0, 0, 0.5); z-index: 999; display: none;">
	<div
		style="text-align: center; position: absolute; background-color: #FFFFFF; left: 50%; top: 50%; height: auto; width: auto; padding: 50px; margin-left: -100px; margin-top: -100px;">
		<style>
.btn-x {
	margin: 10px;
}
